...
for (i, ingredient) in enumerate(l_ingredients)
    if all(x -> x == 0, allergen_matrix[:,i])
        ...
        allergen_index = findfirst(isequal(1), allergen_matrix[:,i])
        push!(dangerous_ingredients, (l_allergens[allergen_index], ingredient))
    end
end

@show join(getindex.(sort(dangerous_ingredients), 2), ",")
